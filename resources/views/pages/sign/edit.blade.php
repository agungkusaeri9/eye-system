    ...
    <div class="col-12">
      <div class="card mb-34">
        <h5 class="card-header">
            {{ $title }}
            <br />
            <small><em class="text-danger">( {{ $span }} )</em></small><br />
            <small><em class="text-danger">Alasan Ditolak : <strong>{{ $sign->alasan_ditolak ? $sign->alasan_ditolak : 'Tidak ada alasah ditolak silahkan hubungi validator jika ingin mengetahuinya' }}</strong></em></small>
        </h5>
        <div class="card-body">
          <div class="row">

            <div class="col-xl-12 col-md- col-sm-12 mb-3">
                <label class="form-label" for="creditCardMask">Nama E-Sign</label>
                <div class="input-group input-group-merge">
                  <input type="text" id="e_sign_name" class="form-control" name="e_sign_name" placeholder="Nama E-Sign" value="{{ $sign->name ?? old('e_sign_name') }}" />
                </div>
                @error('e_sign_name')
            ...
            <div class="col-xl-12 col-md- col-sm-12 mb-3">
                <label class="form-label" for="creditCardMask">File Berita Acara</label><br />
                <small><em>File Sebelumnya :</em> <br /><a href="{{ asset('storage/'.$sign->sign_file) }}" target="__blank"><strong>{{ basename($sign->sign_file) }}</strong></a></small>
                <div class="input-group input-group-merge">
                  <input @if(($sign->signature == 'Ditolak') && ($sign->sign == 'Diterima')) disabled @endif type="file" id="news_file" class="form-control" name="news_file" placeholder="File Berita Acara" accept=".pdf, .doc, .docx, .png, .jpg, .jpeg" />
                </div>
                @error('news_file')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            ...
            <div class="col-xl-12 col-md- col-sm-12 mb-3">
                <label class="form-label" for="creditCardMask">File Untuk Diverifikasi</label><br />
                <small><em>File Sebelumnya :</em> <br /><a href="{{ asset('storage/'.$sign->verified_sign_file) }}" target="__blank"><strong>{{ basename($sign->verified_sign_file) }}</strong></a></small>
                <div class="input-group input-group-merge">
                  <input type="file" id="verification_file" class="form-control" name="verification_file" placeholder="File Verifikasi" accept=".pdf, .doc, .docx, .png, .jpg, .jpeg" />
                </div>
            ...
            <div class="col-12 d-none" id="show-support-document">
                <label class="form-label" for="creditCardMask">Dokumen Pendukung Sebelumnya</label>
                <div class="row" id="form-pendukung-data">
                    {{-- @foreach ($sign->support as $supp)
                        <div class="col-12 mb-3 form-support">
                            <div class="row">
                                <div class="col-md-11 col-9">
                                    <div class="input-group input-group-merge">
                                        <a href="{{ asset('storage/'.$supp->sign_document) }}" target="__blank" type="text" class="form-control text-primary fw-bold">{{ basename($supp->sign_document) }}</a>
                                    </div>
                                </div>
                                <div class="col-md-1 col-3">
                                    <button type="button" onclick="deleteDocument({{ $supp->id }})" class="btn btn-danger"><i class="fas fa-trash"></i></button>
                                </div>
                            </div>
                ...
                <div class="row" id="form-container">
                    <div class="col-12 mb-3 form-pendukung">
                        <div class="row">
                            <div class="col-md-11 col-9">
                                <div class="input-group input-group-merge">
                                    <input type="file" class="form-control" name="supporting_file[]" placeholder="Dokumen Pendukung" accept=".xlx, .xlxs, .docx, .doc, .pdf" />
                                </div>
                            </div>
                            <div class="col-md-1 col-3">
            ...
            <div class="col-12 mb-3">
                ...
                <input type="hidden" class="" id="validator_data_input" name="validator_data" value="{{ $user_data }}">
                @error('to')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
          ...
          {{-- <div class="row" id="validator-data">
            ...
            <div class="col-xl-3 col-md- col-sm-12 mb-3">
                <label class="form-label" for="creditCardMask">Jabatan</label>
                 <div class="input-group input-group-merge">
                  <input type="text" id="position" class="form-control" name="position" placeholder="Jabatan Validator" value="{{ isset($user) ? $user->jabatan->name : '' }}" disabled />
                </div>
            </div>
            <div class="col-xl-3 col-md- col-sm-12 mb-3">
                <label class="form-label" for="creditCardMask">Departemen</label>
                <div class="input-group input-group-merge">
                  <input type="text" id="department" class="form-control" name="department" placeholder="Departemen Validator" value="{{ isset($user) ? $user->departemen->name : '' }}" disabled />
                </div>
            </div>
            <div class="col-xl-3 col-md- col-sm-12 mb-3">
                <label class="form-label" for="creditCardMask">Distrik</label>
                <div class="input-group input-group-merge">
                  <input type="text" id="district" class="form-control" name="district" placeholder="Distrik Validator" value="{{ isset($user) ? $user->distrik->name : '' }}" disabled />
                </div>
            </div>
          </div> --}}
...
@section('page-script')
    <script>
        ...
        function getDocument(){
            $('#form-pendukung-data').empty()
            $.ajax({
                url: '/sign/dokumen-pendukung/{{ $sign->id }}',
                method: "GET",
                processType: true,
                success: function(response){
                    if(response.length > 0) {
                        $('#show-support-document').removeClass('d-none')
                        response.forEach(item => {
                            var href = "{{ asset('storage/') }}"+item['sign_document'];
                            var basename = href.split('/').pop();

                            const html =    `<div class="col-12 mb-3 form-support">`+
                                                `<div class="row">`+
                                                    `<div class="col-md-11 col-9">`+
                                                        `<div class="input-group input-group-merge">`+
                                                            `<a href="${href}" target="__blank" type="text" class="form-control text-primary fw-bold">${basename}</a>`+
                                                        `</div>`+
                                                    `</div>`+
                                                    `<div class="col-md-1 col-3">`+
                                                        `<button type="button" onclick="deleteDocument(${item['id']})" class="btn btn-danger"><i class="fas fa-trash"></i></button>`+
                                                    `</div>`+
                                                `</div>`+
                                            `</div>`;
                            $('#form-pendukung-data').append(html)
                        });
                    } else {
                        $('#show-support-document').addClass('d-none')
                    }
                },
        ...
        function deleteDocument(id){
            Swal.fire({
                title: "Hapus dokumen?",
                text: "Hapus dokumen bersifat permanen, ingin melanjutkan?",
                icon: "warning",
                ...
                if(result.isConfirmed) {
                    $.ajax({
                        url: '/sign/hapus-dokumen-pendukung/'+id,
                        method: 'GET',
                        success: function(response){
                            Swal.fire({
